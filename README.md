...
3. *JSON Hijacking* — Haack/Grossman-era write-ups on the attack class the `for (;;);` prefix defeats; see `CSRFGuard` in [Part 2](02-async-request-anatomy.md).
4. JSON5 specification — `json5.org` (the vendored fallback parser in [Part 1](01-ghost-owl-anti-tampering.md)).
5. Alea PRNG — Johannes Baagøe, 2010 (the seeded PRNG behind `__s` session IDs, [Part 2](02-async-request-anatomy.md)).
6. OWASP — *Prototype Pollution* (the `__proto__`/`hasOwnProperty` query-key defense in [Part 4](04-uri-scheme-allowlist.md)).
...
`facebook` `reverse-engineering` `javascript-internals` `security-research` `haste` `bootloader` `ghost-owl` `anti-tampering` `adblock-detection` `graphql` `csrf` `serverjs` `hyperion` `web-recon` `osint` `meta`
